...
- Custódia offline iniciada: o recovery material foi dividido em `unseal-shares` (5 shares, threshold 3) e `root-token`, cifrados com AES256 e uma passphrase própria do kit, enviados para `gdrive:ValorDigital-Backups/openbao-recovery` e validados por hash + decrypt round-trip. Pendência humana: mover `/root/.valorbrain-private/openbao/kit/kit.pass` para gerenciador de senhas/papel e apagar do host.
