...
- Push concluído em 2026-09-23 para os cinco endereços: engine `main` + tag `v1.1.0`, saas `main` + `v0.15.3`, docs `main` + `v0.0.23`, ops `master` + `v1.0.1`, platform `master` + `v1.0.0`. O versionamento SemVer 2.0.0 foi formalizado em `/www/valorbrain-platform/VERSIONING.md`, com `VERSION`/`CHANGELOG.md` por componente. O token inline do remote do engine foi removido do `.git/config`.
- Custódia offline concluída: o recovery material foi dividido em `unseal-shares` (5 shares, threshold 3) e `root-token`, cifrados com AES256 e uma passphrase própria do kit, enviados para `gdrive:ValorDigital-Backups/openbao-recovery` e validados por hash + decrypt round-trip. A passphrase foi copiada pelo operador para custódia offline e a cópia do host foi removida com `shred`; o root token foi mantido no Drive conforme decisão explícita.
