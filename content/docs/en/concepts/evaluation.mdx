...
Numbers on this page come from the engine's benchmark register; the public mirror lives on the [product benchmarks page](https://valorbrain.valor.digital/en/benchmarks). They are retrieval-evidence numbers unless labeled otherwise — treating them as answer accuracy is a category error (see below).
